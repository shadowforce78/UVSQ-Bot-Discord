...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    if (
      !/^\d{4}-\d{2}-\d{2}$/.test(dateDebut) ||
      !/^\d{4}-\d{2}-\d{2}$/.test(dateFin)
    ) {
    ...
    const postdata =
      `start=${dateDebut}&end=${dateFin}&resType=103&calView=agendaWeek&federationIds%5B%5D=${classe}&colourScheme=3`;
